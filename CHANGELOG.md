...
+ [新增] 新增托盘守护程序 `无限画布托盘.exe`：无控制台窗口不会被误关，每 4 秒自检 `/status`，服务进程退出或自检连续失败都会自动重启；托盘菜单可打开页面、重启服务、查看日志、打开项目目录、设置开机自启。源码在 `tray/`，改完双击 `tray\build.bat` 重新编译（用 Windows 自带的 csc，无需安装编译器）。
