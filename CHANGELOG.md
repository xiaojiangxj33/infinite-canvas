...
+ [修复] 托盘控制台的滚动条改为深色：系统默认浅色轨道在日志卡片右侧形成一条 17px 宽的白色竖条，改用 SetWindowTheme 深色主题后消失，滑块仍可辨识。
+ [修复] 托盘控制台去掉外围浅色标题栏与窗口边框：改用 DwmSetWindowAttribute 深色标题栏，标题栏区域近白像素由 32,453 降到 15。
+ [调整] 托盘控制台按「发布会主题」重做：唯一电蓝 #2997FF 强调色 + 近白/中灰、hero 巨字、规格排成一行（标签+值）、日志为全窗口唯一一处 #1D1D1F 圆角卡片、自绘胶囊按钮（300ms 缓出、按下缩放、焦点环、空格可触发）；动效跟随 Windows 的「显示动画」开关降级。舞台底色按要求改为 #171717（与画布卡片同色，属对「纯黑舞台」的有意偏离）。
+ [修复] 托盘控制台：hero 文本改用自绘控件（Label 在自绘窗口里会整块涂白看不见）；按钮行改为可换行，修掉窗口变窄时的横向溢出；日志去掉横向滚动条改为自动换行。
+ [新增] 新增托盘守护程序 `无限画布托盘.exe`，并提供「无限画布 · 控制台」窗口：窗口顶部是实时状态（运行中 / 自检异常 / 正在重启、自检间隔、已自动重启次数），中间是滚动日志，底部有打开页面、重启服务、清空日志、打开日志文件、缩到托盘与开机自启开关；**关掉窗口只缩回托盘，服务不中断**（实测关闭前后服务进程号不变），彻底退出走托盘菜单。守护部分每 4 秒自检 `/status`，服务进程退出或自检连续失败都会自动重启；日志同时写进 `tray.log`。源码在 `tray/`，改完双击 `tray\build.bat` 重新编译（用 Windows 自带的 csc，无需安装编译器）。
+ [调整] 暗色模式页面底色由 `#0a0a0a` 调整为 `#1f1f1f`（与序列帧生成器一致），导航栏单独保持原 `#0a0a0a`。
+ [调整] 移除导航栏右侧的文档入口、中英文切换、版本号和 GitHub 图标，只保留配置与主题切换；语言固定中文。
