...
**推荐：双击 `无限画布托盘.exe`。** 它把服务交给托盘看护，并提供一个控制台窗口：

- **启动时显示「无限画布 · 控制台」窗口**：顶部是实时状态（运行中 / 自检异常 / 正在重启）、自检间隔与已自动重启次数；中间是滚动日志；底部有「打开无限画布 / 序列帧生成器 / 重启服务 / 清空日志 / 打开日志文件 / 缩到托盘」和一键开关开机自启
- **关掉这个窗口只是缩回右下角托盘，服务照常运行**，连重启都不会发生（这是它和原来那个黑窗口最大的区别 —— 黑窗口一点叉服务就没了）。想彻底退出请右键托盘图标 →「退出（停止服务）」
- **每 4 秒自检一次** `http://127.0.0.1:3000/status`，连续 3 次失败、或服务进程退出，都会**自动重启**；日志里能看到每次重启的原因
- **托盘图标**：双击打开控制台；右键可「打开无限画布 / 打开序列帧生成器 / 立即重启服务 / 打开日志文件 / 打开项目目录 / 开机自动启动（默认关）/ 退出」
- 日志同时写进项目根目录的 `tray.log`，方便事后排查

也可以双击 **`启动无限画布.bat`**：缺少构建产物时会先自动构建，但它占用一个真正的控制台窗口 —— 关掉窗口即停止服务，且不会自动重启。
